<div class="flex items-center justify-center w-full" wire:poll>
    <img class="w-full sm:w-full md:w-2/3 lg:w-1/4" src="{{ asset('storage/' . $task->image_path) }}" alt="">
    <div>
        @if($status === "waiting")
        ...
        @elseif($status === "computing")
            <button type="button" wire:click="setium">МОК: отметить выполненным</button>
            <h2 class="mb-2 text-lg font-semibold text-gray-900 dark:text-white">Обработка запроса:</h2>
            <ul class="max-w-md space-y-2 text-gray-500 list-inside dark:text-gray-400">
                <li class="flex items-center">
